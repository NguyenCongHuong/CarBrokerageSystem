...
function getDayString2(lunar, solarDay, solarMonth, solarYear) {
	var s;
	var dayOfWeek = TUAN[(lunar.jd + 1) % 7];
	var can = (solarYear+6) % 10;
	var chi = (solarYear+8) % 12;
	s =  lunar.day + "/" +lunar.month + "/"+lunar.year;
	return s;
}
...
function alertDayInfo2(dd, mm, yy, leap, jd, sday, smonth, syear) {
	var lunar = new LunarDate(dd, mm, yy, leap, jd);
	var s = getDayString2(lunar, sday, smonth, syear);

	var userName = document.getElementById("giaTri2");
